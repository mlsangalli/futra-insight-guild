...
BEGIN
  -- Check market exists and is not already resolved
  ...
  -- Idempotency guard: if predictions already processed, return early
  IF EXISTS (SELECT 1 FROM predictions WHERE market_id = p_market_id AND status IN ('won', 'lost') LIMIT 1) THEN
